...
using PoolAllocator = doost::DownwardPoolAllocator<unsigned>;
using PoolList = doost::List<unsigned, PoolAllocator>;

static inline void create_list(PoolList& list, unsigned n) {
...
static inline void test(unsigned n) {
  ...
  std::size_t pool_used = 0;
  {
    ...
    {
      PoolList list{PoolAllocator(pool)};
      create_list(list, n);
  ...
  const auto mem_required = static_cast<uint64_t>(n) * PoolList::node_size;
  std::cout << "Node storage required: " << mem_required << " bytes\n";
  std::cout << "Pool usable storage: " << pool_usable << " bytes\n";
  std::cout << "Pool used storage: " << pool_used << " bytes\n";
